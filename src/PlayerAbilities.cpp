...
PlayerAbilities::PlayerAbilities(ASGE::Renderer* renderer, Player& _player, float x, float y) :
  ...
  button_icons(
    std::array<Sprite, 3>{ Sprite(renderer, "data/images/ui/controller/buttons.png", 0),
                           ...
                           Sprite(renderer, "data/images/ui/controller/buttons.png", 0) })
{
  ...
  for (size_t i = 0; i < 3; i++)
  {
    ...
    button_icons[i].position(
      ASGE::Point2D(position.x + (static_cast<float>(30 * i)), position.y - 40));
  }
}
