...
PlayerScore::PlayerScore(ASGE::Renderer* renderer, Player& _player, float x, float y) :
  position(ASGE::Point2D(x,y)),
  player(_player),
  score_card(renderer,"data/images/ui/score_card.png", position),
  score_text(std::array<Text, 6>{ Text(renderer, "", ASGE::Point2D(), FONTS::PIXEL, ASGE::COLOURS::WHITE, 0.8F),
                                  ...
                                  Text(renderer, "", ASGE::Point2D(), FONTS::PIXEL, ASGE::COLOURS::WHITE, 0.6F)})
{
  ...
  score_text[4].position(ASGE::Point2D(score_text[2].position().x + score_text[2].getWidth() - score_text[3].getWidth(),315)); //NEMESIS
...
void PlayerScore::update(InputTracker& input, float dt)
{
  ...
  score_text[1].contents("Kills: " + std::to_string(player.getScore().kills) +
  ...
  //{
    ...
    score_text[3].position(ASGE::Point2D((score_text[2].position().x + score_text[2].getWidth()) - score_text[3].getWidth(), position.y));
  ...
  if (player.getScore().deaths > 0 )
  {
    ...
    score_text[5].position(ASGE::Point2D((score_text[2].position().x + score_text[2].getWidth()) - score_text[5].getWidth(), 345));
  }
