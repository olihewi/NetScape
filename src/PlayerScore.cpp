...
PlayerScore::PlayerScore(ASGE::Renderer* renderer, Player& _player, float x, float y) :
  ...
  score_text(std::array<Text, 3>{ Text(renderer, "", ASGE::Point2D(), FONTS::PIXEL, ASGE::COLOURS::WHITE, 0.8F),
...
void PlayerScore::update(InputTracker& input, float dt)
{
  ...
  //INPUT
  auto score_sheet_button = input.getControllerButtonDown(player.getID(), CONTROLLER::BUTTONS::BACK);
  //UPDATE TEXT
  ...
  int accuracy = static_cast<int>((player.getScore().hit / (player.getScore().hit + player.getScore().miss)) * 100);
  if (accuracy < 0 )
  {
    accuracy = 0;
  }
