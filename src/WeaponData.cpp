...
WeaponData::WeaponData(nlohmann::json j) :
  ...
  is_automatic(j["is_automatic"].get<bool>()), damage(j["damage"].get<float>()),
  max_ammo(j["max_ammo"].get<int>()), ammo_reserve(j["ammo_reserve"].get<int>()),
  fire_rate(j["fire_rate"].get<float>()), reload_time(j["reload_time"].get<float>()),
  recoil(j["recoil"].get<float>()), recoil_regain(j["recoil_regain"].get<float>()),
  inaccuracy(j["inaccuracy"].get<float>()), look_distance(j["look_distance"].get<float>()),
